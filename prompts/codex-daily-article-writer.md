...
- Brand ideas in fresh words: when an article explains a recurring idea (for example a signature phrase, a framework step, a standard warning), explain it in words written for that article's reader. The same explanatory paragraph appearing in two articles of the batch is what the batch check rejects most often; a single short sentence in passing is fine, a reused paragraph is not. Never fix a duplicate-paragraph rejection by shortening the articles; keep every article at its full length and rewrite the duplicated paragraphs only.
